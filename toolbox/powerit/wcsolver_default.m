function [glb,wcInput,info] = wcsolver_default(mySystem,tUser,U1,x0,pSpec,Opt,systemType)
%% WCSOLVER_DEFAULT Routine
%
% Default and simplest solver for nominal analysis
...
for i = 1:MaxIter
    ...
    switch systemType
        case 'tvss'
            [thisY,thisX] = tvlsim(mySystem,thisU,tUser,x0,tvopt);
            Tgrid1 = thisY.Time;
        ...
        case 'polysys'
            [tempInData,tempInTime] = reshapedata(thisU);
            [Tgrid1,x,y]  = sim(mySystem,tUser,x0,[tempInTime tempInData],OdeOptions,OdeSolver);
            [thisY,thisX] = double2tvmat(y,x,Tgrid1);
            
        case 'sfunc'
            [Tgrid1,thisX,thisY] = simsfunc(mySystem,tUser,thisU,x0,tvopt);
    end
    ...
    if (i > 1) && (i < MaxIter)
        ...
        if abs(thisPerf-prevPerf) <= StopTol*gMax
            ...
            [StopTolSatisfied,deltaU] = stopcond(StopTol,thisU,prevU);
            if StopTolSatisfied
                break;
            end
        end
    ...
    switch Display
        case 'on'
            fprintf(dispstr(i,thisPerf,deltaU));
            fprintf(newline);
            
        case 'plot'
            myWatcher = updatewatch(myWatcher,thisU,dispstr(i,thisPerf,deltaU));
...
switch Display
    case 'on'
        ...
        elseif StopTolSatisfied
            fprintf(dispstr(i,thisPerf,deltaU));fprintf(newline);
