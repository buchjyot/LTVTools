function [glb,wcInput,info] = wcsolver_separate_initialcondition_inputnorm_constraint(mySystem,tUser,U1,x0,pSpec,Opt,systemType)
%% WCSOLVER Routine
...
if fixedIC
    DisplayString = @(in1,in2,in3,in4) ltvutil.powerit.dispstr(in1,in2,in3,[]);
else
    DisplayString = @(in1,in2,in3,in4) ltvutil.powerit.dispstr(in1,in2,in3,in4);
end
...
for i = 1:MaxIter
    ...
    lam0r  = lam0(~zeroIC);
    nlam0  = sqrt(lam0r'*iE0*lam0r); %#ok<MINV>
    ...
    if (i > 1) && (i < MaxIter)
        ...
        if abs(thisPerf-prevPerf) <= StopTol*gMax
            ...
            [StopTolSatisfied,deltaU,dX0] = ltvutil.powerit.stopcond(StopTol,thisU,prevU,thisX0,prevX0);
            if StopTolSatisfied
                break;
            end
        end
    ...
    switch Display
        case 'on'
            fprintf(DisplayString(i,thisPerf,deltaU,dX0));
            fprintf(newline);
            
        case 'plot'
            myWatcher = updatewatch(myWatcher,thisU,DisplayString(i,thisPerf,deltaU,dX0));
    ...
    thisU = thisYa*(InputL2Norm/nYa);
    
    % Update initial condition and normalize
    if ~fixedIC
        x0 = iE0*lam0r/nlam0; %#ok<MINV>
        thisX0 = x0;
        thisX0(zeroIC) = zeros(nzIC,1);
    end
end
...
switch Display
    case 'on'
        ...
        elseif StopTolSatisfied
            fprintf(DisplayString(i,thisPerf,deltaU,dX0));fprintf(newline);
