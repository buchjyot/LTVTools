function [g,X11,P,Pdot,Y,lamP] = tvfhlmi(G,v,p,Ps,Psdot,Pm,Pmdot,Opt,NE)
...
if v~=0
    error('XXX - This function requires v=0');
end
...
[AAe,BBe,CCe,DDe,CCeT,Nz,~,Nd] = ltvutil.extsystem(G,v,p,NE);
n = size(AAe,1);
CC1e = CCe(1:Nz,1:n);
...
for i=1:Nt
    ...
    Psdoti = Psdot.Data(:,:,i);
    for j=1:Ns
        lmiterm([i 1 1 Y(j)],[Psi(j)*eye(n); zeros(Nd+1,n)],[AA BB],'s');
